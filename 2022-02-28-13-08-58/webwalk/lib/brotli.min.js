...
oa,"AAAAKKLLKKKKKJJIHHIHHGGFF");(function(b,d){if(31<d.length)throw"sizeBits length must be at most 31";for(var c=0;4>c;++c)if(0!=d[c])throw"first 4 must be 0";db.set(d.subarray(0,0+d.length),0);var e=0,f=b.length;for(c=0;c<d.length;++c){cb[c]=e;var g=db[c];if(0!=g){if(31<=g)throw"newSizeBits values must be less than 31";e+=c<<g;if(0>=e||e>f)throw"newSizeBits is inconsistent: overflow";}}for(c=d.length;32>c;++c)cb[c]=e;if(e!=f)throw"newSizeBits is inconsistent: underflow";bb=b})(na,oa);d.BrotliDecode=
function(b,d){var c=new Zb,e=new l(b);if(0!=c.runningState)throw"State MUST be uninitialized";c.blockTrees=new Int32Array(3091);c.blockTrees[0]=7;c.distRbIdx=3;var f=p(2147483644,3,120);c.distExtraBits=new Int8Array(f);c.distOffset=new Int32Array(f);c.input=e;c.byteBuffer=new Int8Array(4160);c.accumulator32=0;c.shortBuffer=new Int16Array(2080);c.bitOffset=32;c.halfOffset=2048;c.endOfStreamReached=0;$a(c);c.runningState=1;if(d){var g=d.customDictionary;if(g){if(1!=c.runningState)throw"State MUST be freshly initialized";
0==c.cdNumChunks&&(c.cdChunks=Array(16),c.cdChunkOffsets=new Int32Array(16),c.cdBlockBits=-1);if(15==c.cdNumChunks)throw"Too many dictionary chunks";c.cdChunks[c.cdNumChunks]=g;c.cdNumChunks++;c.cdTotalSize+=g.length;c.cdChunkOffsets[c.cdNumChunks]=c.cdTotalSize}}for(var R=0,h=[];;){var t=new Int8Array(16384);h.push(t);c.output=t;c.outputOffset=0;c.outputLength=16384;c.outputUsed=0;a:{var a=c;if(0==a.runningState)throw"Can't decompress until initialized";if(11==a.runningState)throw"Can't decompress after close";
if(1==a.runningState){var fa=q(a);if(-1==fa)throw"Invalid 'windowBits' code";a.maxRingBufferSize=1<<fa;a.maxBackwardDistance=a.maxRingBufferSize-16;a.runningState=2}for(var Y=yb(a),S=a.ringBufferSize-1,z=a.ringBuffer;10!=a.runningState;)switch(a.runningState){case 2:if(0>a.metaBlockLength)throw"Invalid metablock length";var u=a;if(0!=u.inputEnd)u.nextRunningState=10,u.runningState=12;else{u.literalTreeGroup=new Int32Array(0);u.commandTreeGroup=new Int32Array(0);u.distanceTreeGroup=new Int32Array(0);
2030<u.halfOffset&&G(u);b:{var r=u;16<=r.bitOffset&&(r.accumulator32=r.shortBuffer[r.halfOffset++]<<16|r.accumulator32>>>16,r.bitOffset-=16);r.inputEnd=n(r,1);r.metaBlockLength=0;r.isUncompressed=0;r.isMetadata=0;if(0==r.inputEnd||0==n(r,1)){var Ba=n(r,2)+4;if(7==Ba){r.isMetadata=1;if(0!=n(r,1))throw"Corrupted reserved bit";var Ca=n(r,2);if(0==Ca)break b;for(var L=0;L<Ca;L++){16<=r.bitOffset&&(r.accumulator32=r.shortBuffer[r.halfOffset++]<<16|r.accumulator32>>>16,r.bitOffset-=16);var pa=n(r,8);if(0==
pa&&L+1==Ca&&1<Ca)throw"Exuberant nibble";r.metaBlockLength|=pa<<8*L}}else for(L=0;L<Ba;L++){16<=r.bitOffset&&(r.accumulator32=r.shortBuffer[r.halfOffset++]<<16|r.accumulator32>>>16,r.bitOffset-=16);pa=n(r,4);if(0==pa&&L+1==Ba&&4<Ba)throw"Exuberant nibble";r.metaBlockLength|=pa<<4*L}r.metaBlockLength++;0==r.inputEnd&&(r.isUncompressed=n(r,1))}}if(0!=u.metaBlockLength||0!=u.isMetadata)if(0!=u.isUncompressed||0!=u.isMetadata?(Ab(u),u.runningState=0!=u.isMetadata?5:6):u.runningState=3,0==u.isMetadata&&
(u.expectedTotalSize+=u.metaBlockLength,1073741824<u.expectedTotalSize&&(u.expectedTotalSize=1073741824),u.ringBufferSize<u.maxRingBufferSize)){var M=u,T=M.maxRingBufferSize;if(T>M.expectedTotalSize){for(var xa=M.expectedTotalSize;T>>1>xa;)T>>=1;0==M.inputEnd&&16384>T&&16384<=M.maxRingBufferSize&&(T=16384)}if(!(T<=M.ringBufferSize)){var la=new Int8Array(T+37);0!=M.ringBuffer.length&&la.set(M.ringBuffer.subarray(0,0+M.ringBufferSize),0);M.ringBuffer=la;M.ringBufferSize=T}}}Y=yb(a);S=a.ringBufferSize-
1;z=a.ringBuffer;continue;case 3:var k=a;k.numLiteralBlockTypes=wa(k)+1;k.literalBlockLength=Wa(k,0,k.numLiteralBlockTypes);k.numCommandBlockTypes=wa(k)+1;k.commandBlockLength=Wa(k,1,k.numCommandBlockTypes);k.numDistanceBlockTypes=wa(k)+1;k.distanceBlockLength=Wa(k,2,k.numDistanceBlockTypes);2030<k.halfOffset&&G(k);16<=k.bitOffset&&(k.accumulator32=k.shortBuffer[k.halfOffset++]<<16|k.accumulator32>>>16,k.bitOffset-=16);k.distancePostfixBits=n(k,2);k.numDirectDistanceCodes=n(k,4)<<k.distancePostfixBits;
k.contextModes=new Int8Array(k.numLiteralBlockTypes);for(var qa=0;qa<k.numLiteralBlockTypes;){for(var za=K(qa+96,k.numLiteralBlockTypes);qa<za;++qa)16<=k.bitOffset&&(k.accumulator32=k.shortBuffer[k.halfOffset++]<<16|k.accumulator32>>>16,k.bitOffset-=16),k.contextModes[qa]=n(k,2);2030<k.halfOffset&&G(k)}k.contextMap=new Int8Array(k.numLiteralBlockTypes<<6);var Aa=wb(k.numLiteralBlockTypes<<6,k.contextMap,k);k.trivialLiteralContext=1;for(var Ga=0;Ga<k.numLiteralBlockTypes<<6;Ga++)if(k.contextMap[Ga]!=
Ga>>6){k.trivialLiteralContext=0;break}k.distContextMap=new Int8Array(k.numDistanceBlockTypes<<2);var Da=wb(k.numDistanceBlockTypes<<2,k.distContextMap,k);k.literalTreeGroup=Xa(256,256,Aa,k);k.commandTreeGroup=Xa(704,704,k.numCommandBlockTypes,k);var eb=16+k.numDirectDistanceCodes+2*(24<<k.distancePostfixBits),fb=eb;1==k.isLargeWindow&&(eb=16+k.numDirectDistanceCodes+2*(62<<k.distancePostfixBits),fb=p(2147483644,k.distancePostfixBits,k.numDirectDistanceCodes));k.distanceTreeGroup=Xa(eb,fb,Da,k);for(var Ea=
fb,na=k.distExtraBits,oa=k.distOffset,Bb=k.distancePostfixBits,Cb=k.numDirectDistanceCodes,Sa=1<<Bb,gb=1,hb=0,Z=16,aa=0;aa<Cb;++aa)na[Z]=0,oa[Z]=aa+1,++Z;for(;Z<Ea;){var Ta=Cb+((2+hb<<gb)-4<<Bb)+1;for(aa=0;aa<Sa;++aa)na[Z]=gb,oa[Z]=Ta+aa,++Z;gb+=hb;hb^=1}k.contextMapSlice=0;k.distContextMapSlice=0;k.contextLookupOffset1=512*k.contextModes[0];k.contextLookupOffset2=k.contextLookupOffset1+256;k.literalTreeIdx=0;k.commandTreeIdx=0;k.rings[4]=1;k.rings[5]=0;k.rings[6]=1;k.rings[7]=0;k.rings[8]=1;k.rings[9]=
0;a.runningState=4;case 4:if(0>=a.metaBlockLength){a.runningState=2;continue}2030<a.halfOffset&&G(a);if(0==a.commandBlockLength){var ra=a;ra.commandBlockLength=Va(ra,1,ra.numCommandBlockTypes);ra.commandTreeIdx=ra.rings[7]}a.commandBlockLength--;16<=a.bitOffset&&(a.accumulator32=a.shortBuffer[a.halfOffset++]<<16|a.accumulator32>>>16,a.bitOffset-=16);var Ha=X(a.commandTreeGroup,a.commandTreeIdx,a)<<2,Db=ma[Ha],Ua=ma[Ha+1],bc=ma[Ha+2];a.distanceCode=ma[Ha+3];16<=a.bitOffset&&(a.accumulator32=a.shortBuffer[a.halfOffset++]<<
16|a.accumulator32>>>16,a.bitOffset-=16);var J=Db&255;a.insertLength=Ua+(16>=J?n(a,J):ya(a,J));16<=a.bitOffset&&(a.accumulator32=a.shortBuffer[a.halfOffset++]<<16|a.accumulator32>>>16,a.bitOffset-=16);J=Db>>8;a.copyLength=bc+(16>=J?n(a,J):ya(a,J));a.j=0;a.runningState=7;case 7:if(0!=a.trivialLiteralContext)for(;a.j<a.insertLength;){if(2030<a.halfOffset&&G(a),0==a.literalBlockLength&&xb(a),a.literalBlockLength--,16<=a.bitOffset&&(a.accumulator32=a.shortBuffer[a.halfOffset++]<<16|a.accumulator32>>>
16,a.bitOffset-=16),z[a.pos]=X(a.literalTreeGroup,a.literalTreeIdx,a),a.pos++,a.j++,a.pos>=Y){a.nextRunningState=7;a.runningState=12;break}}else for(var Ia=z[a.pos-1&S]&255,Eb=z[a.pos-2&S]&255;a.j<a.insertLength;){2030<a.halfOffset&&G(a);0==a.literalBlockLength&&xb(a);var cc=a.contextMap[a.contextMapSlice+(ab[a.contextLookupOffset1+Ia]|ab[a.contextLookupOffset2+Eb])]&255;a.literalBlockLength--;Eb=Ia;16<=a.bitOffset&&(a.accumulator32=a.shortBuffer[a.halfOffset++]<<16|a.accumulator32>>>16,a.bitOffset-=
16);Ia=X(a.literalTreeGroup,cc,a);z[a.pos]=Ia;a.pos++;a.j++;if(a.pos>=Y){a.nextRunningState=7;a.runningState=12;break}}if(7!=a.runningState)continue;a.metaBlockLength-=a.insertLength;if(0>=a.metaBlockLength){a.runningState=4;continue}var P=a.distanceCode;if(0>P)a.distance=a.rings[a.distRbIdx];else{2030<a.halfOffset&&G(a);if(0==a.distanceBlockLength){var sa=a;sa.distanceBlockLength=Va(sa,2,sa.numDistanceBlockTypes);sa.distContextMapSlice=sa.rings[9]<<2}a.distanceBlockLength--;16<=a.bitOffset&&(a.accumulator32=
a.shortBuffer[a.halfOffset++]<<16|a.accumulator32>>>16,a.bitOffset-=16);P=X(a.distanceTreeGroup,a.distContextMap[a.distContextMapSlice+P]&255,a);if(16>P){if(a.distance=a.rings[a.distRbIdx+$b[P]&3]+ac[P],0>a.distance)throw"Negative distance";}else{J=a.distExtraBits[P];if(32>=a.bitOffset+J)var Fb=n(a,J);else 16<=a.bitOffset&&(a.accumulator32=a.shortBuffer[a.halfOffset++]<<16|a.accumulator32>>>16,a.bitOffset-=16),Fb=16>=J?n(a,J):ya(a,J);a.distance=a.distOffset[P]+(Fb<<a.distancePostfixBits)}}a.maxDistance=
a.maxDistance!=a.maxBackwardDistance&&a.pos<a.maxBackwardDistance?a.pos:a.maxBackwardDistance;if(a.distance>a.maxDistance){a.runningState=9;continue}0<P&&(a.distRbIdx=a.distRbIdx+1&3,a.rings[a.distRbIdx]=a.distance);if(a.copyLength>a.metaBlockLength)throw"Invalid backward reference";a.j=0;a.runningState=8;case 8:var ba=a.pos-a.distance&S,ca=a.pos,da=a.copyLength-a.j,ib=ba+da,Gb=ca+da;if(ib<S&&Gb<S){if(12>da||ib>ca&&Gb>ba)for(var Hb=0;Hb<da;Hb+=4)z[ca++]=z[ba++],z[ca++]=z[ba++],z[ca++]=z[ba++],z[ca++]=
z[ba++];else z.copyWithin(ca,ba,ib);a.j+=da;a.metaBlockLength-=da;a.pos+=da}else for(;a.j<a.copyLength;)if(z[a.pos]=z[a.pos-a.distance&S],a.metaBlockLength--,a.pos++,a.j++,a.pos>=Y){a.nextRunningState=8;a.runningState=12;break}8==a.runningState&&(a.runningState=4);continue;case 9:var D=a,dc=Y;if(2147483644<D.distance)throw"Invalid backward reference";var Ja=D.distance-D.maxDistance-1-D.cdTotalSize;if(0>Ja){var y=D,Ka=-Ja-1,jb=D.copyLength;if(-1==y.cdBlockBits){y.cdBlockMap=new Int8Array(256);for(var ha=
8;0!=y.cdTotalSize-1>>>ha;)ha++;ha-=8;y.cdBlockBits=ha;for(var La=0,kb=0;La<y.cdTotalSize;){for(;y.cdChunkOffsets[kb+1]<La;)kb++;y.cdBlockMap[La>>>ha]=kb;La+=1<<ha}}for(var Ma=y.cdBlockMap[Ka>>>y.cdBlockBits];Ka>=y.cdChunkOffsets[Ma+1];)Ma++;if(y.cdTotalSize>Ka+jb)throw"Invalid backward reference";y.distRbIdx=y.distRbIdx+1&3;y.rings[y.distRbIdx]=y.distance;y.metaBlockLength-=jb;y.cdBrIndex=Ma;y.cdBrOffset=Ka-y.cdChunkOffsets[Ma];y.cdBrLength=jb;y.cdBrCopied=0;D.runningState=14}else{if(null==bb&&!DataLoader.OK)throw"brotli dictionary is not set";
var ec=bb;var ta=D.copyLength;if(31<ta)throw"Invalid backward reference";var lb=db[ta];if(0==lb)throw"Invalid backward reference";var fc=cb[ta],gc=Ja&(1<<lb)-1,Ib=Ja>>>lb,Jb=Fa;if(Ib>=Jb.numTransforms)throw"Invalid backward reference";var v=D.ringBuffer,Kb=D.pos,hc=ec,Lb=fc+gc*ta,C=ta,Na=Jb,Mb=Ib,ia=Kb,mb=Na.triplets,Nb=Na.prefixSuffixStorage,Oa=Na.prefixSuffixHeads,nb=3*Mb,Ob=mb[nb],U=mb[nb+1],Pb=mb[nb+2],Qb=Oa[Ob],ic=Oa[Ob+1],Rb=Oa[Pb],jc=Oa[Pb+1],ea=U-11,Pa=U-0;if(1>ea||9<ea)ea=0;if(1>Pa||9<Pa)Pa=
0;for(;Qb!=ic;)v[ia++]=Nb[Qb++];ea>C&&(ea=C);Lb+=ea;for(var Sb=C=C-ea-Pa;0<Sb;)v[ia++]=hc[Lb++],Sb--;if(10==U||11==U){var O=ia-C;for(10==U&&(C=1);0<C;){var H=v[O]&255;192>H?(97<=H&&122>=H&&(v[O]^=32),O+=1,--C):224>H?(v[O+1]^=32,O+=2,C-=2):(v[O+2]^=5,O+=3,C-=3)}}else if(21==U||22==U)for(var E=ia-C,Tb=Na.params[Mb],I=(Tb&32767)+(16777216-(Tb&32768));0<C;){var V=1;H=v[E]&255;if(128>H)I+=H,v[E]=I&127;else if(!(192>H))if(224>H)if(2<=C){var W=v[E+1];I+=W&63|(H&31)<<6;v[E]=192|I>>6&31;v[E+1]=W&192|I&63;
V=2}else V=C;else if(240>H)if(3<=C){W=v[E+1];var ua=v[E+2];I+=ua&63|(W&63)<<6|(H&15)<<12;v[E]=224|I>>12&15;v[E+1]=W&192|I>>6&63;v[E+2]=ua&192|I&63;V=3}else V=C;else if(248>H)if(4<=C){W=v[E+1];ua=v[E+2];var Ub=v[E+3];I+=Ub&63|(ua&63)<<6|(W&63)<<12|(H&7)<<18;v[E]=240|I>>18&7;v[E+1]=W&192|I>>12&63;v[E+2]=ua&192|I>>6&63;v[E+3]=Ub&192|I&63;V=4}else V=C;E+=V;C-=V;21==U&&(C=0)}for(;Rb!=jc;)v[ia++]=Nb[Rb++];var Vb=ia-Kb;D.pos+=Vb;D.metaBlockLength-=Vb;D.pos>=dc?(D.nextRunningState=4,D.runningState=12):D.runningState=
4}continue;case 14:for(var kc=a,lc=a.pos,A=a,Wb=Y,ja=A.pos,mc=ja;A.cdBrLength!=A.cdBrCopied;){var Xb=Wb-ja,ob=A.cdChunkOffsets[A.cdBrIndex+1]-A.cdChunkOffsets[A.cdBrIndex]-A.cdBrOffset,Q=A.cdBrLength-A.cdBrCopied;Q>ob&&(Q=ob);Q>Xb&&(Q=Xb);var nc=ja;A.ringBuffer.set(A.cdChunks[A.cdBrIndex].slice(A.cdBrOffset,A.cdBrOffset+Q),nc);ja+=Q;A.cdBrOffset+=Q;A.cdBrCopied+=Q;Q==ob&&(A.cdBrIndex++,A.cdBrOffset=0);if(ja>=Wb)break}kc.pos=lc+(ja-mc);if(a.pos>=Y){a.nextRunningState=14;a.runningState=12;break a}a.runningState=
4;continue;case 5:for(;0<a.metaBlockLength;)2030<a.halfOffset&&G(a),16<=a.bitOffset&&(a.accumulator32=a.shortBuffer[a.halfOffset++]<<16|a.accumulator32>>>16,a.bitOffset-=16),n(a,8),a.metaBlockLength--;a.runningState=2;continue;case 6:var B=a,oc=B.ringBuffer;if(0>=B.metaBlockLength)32==B.bitOffset&&$a(B),B.runningState=2;else{var pb=K(B.ringBufferSize-B.pos,B.metaBlockLength),w=B,Qa=oc,ka=B.pos,N=pb;if(0!=(w.bitOffset&7))throw"Unaligned copyBytes";for(;32!=w.bitOffset&&0!=N;)Qa[ka++]=w.accumulator32>>>
w.bitOffset,w.bitOffset+=8,N--;if(0!=N){var qb=K(Ya(w),N>>1);if(0<qb){var Yb=w.halfOffset<<1,rb=qb<<1;Qa.set(w.byteBuffer.subarray(Yb,Yb+rb),ka);ka+=rb;N-=rb;w.halfOffset+=qb}if(0!=N)if(0<Ya(w)){16<=w.bitOffset&&(w.accumulator32=w.shortBuffer[w.halfOffset++]<<16|w.accumulator32>>>16,w.bitOffset-=16);for(;0!=N;)Qa[ka++]=w.accumulator32>>>w.bitOffset,w.bitOffset+=8,N--;Za(w,0)}else for(;0<N;){var sb=zb(w.input,Qa,ka,N);if(-1==sb)throw"Unexpected end of input";ka+=sb;N-=sb}}B.metaBlockLength-=pb;B.pos+=
pb;B.pos==B.ringBufferSize?(B.nextRunningState=6,B.runningState=12):(32==B.bitOffset&&$a(B),B.runningState=2)}continue;case 12:a.ringBufferBytesReady=K(a.pos,a.ringBufferSize),a.runningState=13;case 13:var F=a,Ra=K(F.outputLength-F.outputUsed,F.ringBufferBytesReady-F.ringBufferBytesWritten);0!=Ra&&(F.output.set(F.ringBuffer.subarray(F.ringBufferBytesWritten,F.ringBufferBytesWritten+Ra),F.outputOffset+F.outputUsed),F.outputUsed+=Ra,F.ringBufferBytesWritten+=Ra);if(0==(F.outputUsed<F.outputLength?1:
0))break a;a.pos>=a.maxBackwardDistance&&(a.maxDistance=a.maxBackwardDistance);a.pos>=a.ringBufferSize&&(a.pos>a.ringBufferSize&&z.copyWithin(0,a.ringBufferSize,a.pos),a.pos&=S,a.ringBufferBytesWritten=0);a.runningState=a.nextRunningState;continue;default:throw"Unexpected state "+a.runningState;}if(10==a.runningState){if(0>a.metaBlockLength)throw"Invalid metablock length";Ab(a);Za(a,1)}}R+=c.outputUsed;if(16384>c.outputUsed)break}if(0==c.runningState)throw"State MUST be initialized";11!=c.runningState&&
(c.runningState=11,null!=c.input&&(c.input=null));for(var tb=new Int8Array(R),va=0,ub=0;ub<h.length;++ub){t=h[ub];var vb=K(R,va+16384)-va;16384>vb?tb.set(t.subarray(0,vb),va):tb.set(t,va);va+=vb}return tb}})(window);
